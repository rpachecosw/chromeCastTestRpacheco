...
//
//  Created by Richard Pacheco on 4/6/22.
//
...
class CastManager: NSObject {
    ...
    private func initialiseContext() {
        ...
        let options = GCKCastOptions(discoveryCriteria: GCKDiscoveryCriteria.init(applicationID: "B0E88C81"))
    ...
    private func miniControllerStyle() {
        let castStyle = GCKUIStyle.sharedInstance()
        castStyle.castViews.mediaControl.miniController.backgroundColor = .black
    ...
    func buildMediaInformation(with title: String, with description: String, with studio: String, with duration: TimeInterval, with movieUrl: String, with streamType: GCKMediaStreamType, with thumbnailUrl: String?) -> GCKMediaInformation {
        ...
        let mediaInfo = GCKMediaInformation.init(contentID: movieUrl, streamType: streamType, contentType: "video/mp4", metadata: metadata, streamDuration: duration, mediaTracks: nil, textTrackStyle: nil, customData: nil)
    ...
    func startSelectedItemRemotely(_ mediaInfo: GCKMediaInformation, at time: TimeInterval, completion: (Bool) -> Void) {
        guard let remoteMediaClient = sessionManager.currentCastSession?.remoteMediaClient else {return completion(false) }

            let mediaQueueItemBuilder = GCKMediaQueueItemBuilder()
            mediaQueueItemBuilder.mediaInformation = mediaInfo
            mediaQueueItemBuilder.autoplay = true
            mediaQueueItemBuilder.preloadTime = TimeInterval(5)
...
extension CastManager: GCKSessionManagerListener, GCKRequestDelegate {
