...
final class PlayerViewModel: ObservableObject {
    ...
    func setCurrentItem(_ item: Media) {
        currentTime = .zero
        duration = nil
        player.replaceCurrentItem(with: item.asPlayerItem)
        currentItem = item
        
        item.asPlayerItem.publisher(for: \.status)
            .filter({ $0 == .readyToPlay })
            .sink(receiveValue: { [weak self] _ in
                self?.duration = item.asPlayerItem.asset.duration.seconds
            })
            .store(in: &subscriptions)
    }
}
